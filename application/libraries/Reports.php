...
class Reports    extends BaseRow
{
	public function get_table_cols()
	{
		return array('date'=>'Дата','url'=>'Ссылка' ); 
	} 
	
	public function get_table_cols_template()
	{
		return array('url'=>'<a href="[val]" target="_blank" >[val]</a>'); 
	} 
	...
	public function generate_form_rows($class='')
	{
		...
		//простые поля  
		$rows=array('name'=>'text','url'=>'text',  'count'=>'text' );
		$placeholder=array('name'=>'Подарок','url'=>'Ссылка для виртуальных подарков','virtual'=>'Виртуальный?', 'count'=>'Осталось');
		$form=array();
		foreach ($rows as $k=>$v) {
		...
		//со справочниками
		$rows=array('virtual'=>array(0=>'Нет',1=>'Да') );
		foreach ($rows as $k=>$v) {
